...
export default function ContractorUpload({ jobId, apiBase, token, stage }) {
  ...
  return (
    <div style={{ background:B.white, border:"1px solid "+B.tone1, borderRadius:10, padding:"1.25rem", marginBottom:16 }}>
      ...
      <ActionRow label="Delivery file(s)" hint={stage==="working_drawings"?"PDF + DWG for client delivery":"PDF for client delivery"} done={status.deliveryFileDone} doneLabel={status.deliveryFileNames?.length?status.deliveryFileNames.join(", "):"Uploaded"} busy={busyAction==="delivery"} triggerLabel="Upload file(s)" onTrigger={()=>deliveryInputRef.current?.click()} />
