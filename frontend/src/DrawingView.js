                ...
                <div key={c.id} style={{marginBottom:10}}>
                  <div onClick={()=>{setSelectedCid(c.id);setReplyTarget(c.id);}}
                    ...
                    {(c.author?.id===user?.id||user?.role==="admin")&&c.status!=="confirmed"&&<div style={{display:"flex",gap:6,marginTop:6}}>
                      ...
                      <button onClick={e=>{e.stopPropagation();setComments(comments.map(x=>x.id===c.id?{...x,text:""}:x));}} style={{fontSize:10,padding:"2px 8px",border:"1px solid "+B.tone1,borderRadius:4,background:B.white,cursor:"pointer",color:B.black2,fontFamily:"Manrope,sans-serif"}}>Clear</button>
                      <button onClick={e=>{e.stopPropagation();setComments(comments.filter(x=>x.id!==c.id));}} style={{fontSize:10,padding:"2px 8px",border:"1px solid #E24B4A",borderRadius:4,background:B.white,cursor:"pointer",color:"#8B2020",fontFamily:"Manrope,sans-serif"}}>Delete pin</button>
                    </div>}
                  </div>
                  {(c.replies||[]).map(r=>(
                    <div key={r.id} style={{marginLeft:12,marginTop:5,padding:"8px 10px",background:r.author?.role==="team"||r.author?.role==="admin"||r.author?.role==="contractor"?"#FEF3E8":B.white,border:"1px solid "+(r.author?.role==="team"||r.author?.role==="admin"||r.author?.role==="contractor"?B.orange:B.tone1),borderRadius:7,borderLeft:"3px solid "+(r.author?.role==="team"||r.author?.role==="admin"||r.author?.role==="contractor"?B.orange:B.tone2)}}>
                      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span style={{fontSize:11,fontWeight:600,color:(r.author?.role==="team"||r.author?.role==="admin"||r.author?.role==="contractor")?B.orange:B.black}}>{r.author?.role==="contractor"?"Xpress Draft":r.author?.name}</span>{r.is_ai_interpreted&&<span style={{fontSize:9,background:B.orange,color:"#fff",borderRadius:4,padding:"1px 5px"}}>AI interpreted</span>}{(r.author?.role==="team"||r.author?.role==="admin"||r.author?.role==="contractor")&&!r.is_ai_interpreted&&<span style={{fontSize:9,background:B.orange,color:"#fff",borderRadius:4,padding:"1px 5px"}}>XD Team</span>}{r.is_private&&<span style={{fontSize:9,background:"#444",color:"#fff",borderRadius:4,padding:"1px 5px"}}>Private</span>}</div>
                  ...
                  {isSelected&&replyTarget===c.id&&c.status!=="confirmed"&&(
                    <div style={{marginLeft:12,marginTop:6}}>
                      ...
                      {isTeam&&<label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:B.black2,marginBottom:4,cursor:"pointer"}}><input type="checkbox" checked={replyIsPrivate} onChange={e=>setReplyIsPrivate(e.target.checked)}/>Private (hidden from client)</label>}
                      <div style={{display:"flex",gap:6,marginTop:5}}>
                        ...
                        <button onClick={sendReply} style={{...btnPrimary,fontSize:11,flex:1,background:replyIsPrivate?"#444":B.orange}}>{replyIsPrivate?"Send private":"Send"}</button>
                      </div>
                    </div>
                  )}
                </div>
