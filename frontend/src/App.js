    ...
    return(
      <div style={{minHeight:"100vh",background:B.white,fontFamily:"Manrope,sans-serif"}}>
        ...
        <div style={{maxWidth:"100%",padding:"2rem 32px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:32}}>{[{label:"Stage",value:statusLoading?"…":projectStatus?.stage||"Setup",bg:"#FEF3E8",color:B.orange,border:"1px solid "+B.tone1},{label:"Revision",value:statusLoading?"…":projectStatus?.revision||"—",bg:B.cream,color:B.black1,border:"1px solid "+B.tone1},{label:"Timeline",value:statusLoading?"…":projectStatus?.timeline||"—",bg:"#EBF3FE",color:"#1A4A8A",border:"1px solid #C5DCF5"}].map((s,i)=>(<div key={i} style={{background:s.bg,border:s.border,borderRadius:12,padding:"20px 16px",textAlign:"center"}}><div style={{fontSize:11,color:B.tone2,fontWeight:600,letterSpacing:"0.08em",marginBottom:8}}>{s.label.toUpperCase()}</div><div style={{fontSize:16,fontWeight:700,color:s.color}}>{s.value}</div></div>))}</div>

          {projectStatus?.timeline&&currentStep>=0&&(<div style={{background:B.cream,borderRadius:12,padding:"20px 24px",marginBottom:32,border:"1px solid "+B.tone1}}><div style={{fontSize:11,color:B.black2,fontWeight:600,letterSpacing:"0.08em",marginBottom:16}}>PROJECT PROGRESS</div><div style={{display:"flex",alignItems:"center",overflowX:"auto"}}>{timelineSteps.map((step,i)=>(<div key={i} style={{display:"flex",alignItems:"center",flex:1,minWidth:0}}><div style={{display:"flex",flexDirection:"column",alignItems:"center",flex:1}}><div style={{width:12,height:12,borderRadius:"50%",background:i<currentStep?"#639922":i===currentStep?B.orange:B.tone1,border:i===currentStep?"2px solid "+B.orange:"none"}}/><div style={{fontSize:8,color:i<=currentStep?B.black:B.black2,marginTop:4,textAlign:"center",maxWidth:48,lineHeight:1.2}}>{step}</div></div>{i<timelineSteps.length-1&&<div style={{height:2,flex:1,background:i<currentStep?"#639922":B.tone1,marginBottom:16,minWidth:8}}/>}</div>))}</div></div>)}
