...
{pendingPin&&<div style={{position:"absolute",left:pendingPin.fx*(markupRef.current?.width||1),top:pendingPin.fy*(markupRef.current?.height||1),transform:"translate(-50%,-50%)",width:26,height:26,borderRadius:"50%",background:B.orange,border:"2px solid white",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:"#fff",zIndex:11,boxShadow:"0 2px 6px rgba(0,0,0,0.4)",pointerEvents:"none",animation:"pulse 1s infinite"}}>📍</div>}
               {pendingPin&&<div style={{position:"absolute",left:pendingPin.fx*(markupRef.current?.width||1),top:pendingPin.fy*(markupRef.current?.height||1),transform:"translate(-50%,-50%)",width:26,height:26,borderRadius:"50%",background:B.orange,border:"2px solid white",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:"#fff",zIndex:11,boxShadow:"0 2px 6px rgba(0,0,0,0.4)",pointerEvents:"none"}}>📍</div>} {comments.filter(c=>c.pin_x!=null).map((c,i)=>{
                ...
                <div key={c.id} style={{marginBottom:10}}>
                  <div onClick={()=>{setSelectedCid(c.id);setReplyTarget(c.id);}}
                    ...
                    <p style={{fontSize:12,color:B.black1,lineHeight:1.55,margin:"0 0 6px"}}>{c.text}</p>
                      {c.author?.id===user?.id&&c.status!=="confirmed"&&<div style={{display:"flex",gap:6,marginTop:4}}>
  <button onClick={()=>{const t=prompt("Edit comment:",c.text);if(t&&t.trim())setComments(comments.map(x=>x.id===c.id?{...x,text:t.trim()}:x));}} style={{fontSize:10,padding:"2px 8px",border:`1px solid ${B.tone1}`,borderRadius:4,background:B.white,cursor:"pointer",color:B.black2,fontFamily:"Manrope,sans-serif"}}>✏ Edit</button>
  <button onClick={()=>{if(window.confirm("Delete this comment?"))setComments(comments.filter(x=>x.id!==c.id));}} style={{fontSize:10,padding:"2px 8px",border:"1px solid #E24B4A",borderRadius:4,background:B.white,cursor:"pointer",color:"#8B2020",fontFamily:"Manrope,sans-serif"}}>🗑 Delete</button>
</div>}
...
function XPDLogo({size=40}){
  const scale = size/188.38;
  return(
   <svg width={size} height={size} viewBox="-2 -2 192 192" xmlns="http://www.w3.org/2000/svg">
