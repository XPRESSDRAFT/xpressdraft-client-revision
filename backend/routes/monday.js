    ...
    try {
      ...
      if (contractor && contractor.email) {
        ...
        if (contractorUser) {
          ...
          // Create contractor_job row if not exists
          const { data: existingJob } = await supabase.from('contractor_jobs').select('id').eq('contractor_id', contractorUser.id).eq('project_id', project.id).single();
          if (!existingJob) { await supabase.from('contractor_jobs').insert({ contractor_id: contractorUser.id, project_id: project.id, status: 'pending' }); console.log(`Created contractor job for ${contractor.name}`); }
        }
      }
    } catch (e) {
